...
GxIO_Class io(SPI, ELINK_CS, ELINK_DC, ELINK_RESET); // arbitrary selection of 17, 16
...
void displayInit(void)
{
    ...
    SPI.begin(ELINK_CLK, ELINK_MISO, ELINK_MOSI, ELINK_CS);
    isInit = true;
    display.init();
...
// test sdcard
    ...
    if (!SD.begin(SDCARD_CS, sdSPI)) {
        ...
        displayText("SDCard init fail", 50, CENTER_ALIGNMENT);


    } else {
        Serial.println("SDCard init pass");
        displayText("SDCard init pass", 50, CENTER_ALIGNMENT);
    }
...
#ifdef USE_SD
    sdSPI.begin(SDCARD_CLK, SDCARD_MISO, SDCARD_MOSI, SDCARD_CS);
    if (!FILESYSTEM.begin(SDCARD_CS, sdSPI))
#else
    if (!FILESYSTEM.begin())
#endif
    {
